...
def main():
    ...
    if index_category == '4' and type_index == '3':
        hzgc_selection = input('''请在放入国抽导入表格并按【→1 导入】;【按→2 终止】''')
        hzgc_judge = True
        while hzgc_judge:
            if hzgc_selection == '1':                                
                save_hzgc()                
                print('已完成当前数据表格导入！')
                hzgc_selection = input('''请在放入国抽导入表格并按【→1 导入】;【按→2 终止】(按ENTER确认):''')
            elif hzgc_selection == '2':
                hzgc_judge = False
    elif index_category == '4' and type_index == '1':
        jkhz_selection = input('请在放入申请表扫描件后按【→1 导入】;【按→2 终止】(按ENTER确认):')
        jkhz_judge = True
        while jkhz_judge:
            if jkhz_selection == '1':
                save_hzjkft()
